...
function StepPill({ icon, label }: StepPillProps) {
    return (
        <motion.div
...
const CustomResponse = ({ children }: { children: React.ReactNode }) => {
    return (
        <Response
            components={{
                code: ({ children }) => (
...
export const ChatDemo = ({
    ...
    }, [conversation, cursor, max, playing]);

    const visibleItems = useMemo(() => conversation.slice(0, cursor), [conversation, cursor]);
